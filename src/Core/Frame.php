...
class Frame
{
    ...
    public static function buildDataDown(
        ...
        bool $confirmed, bool $ack, ?int $fport, string $payload, int $adr = 0
    ): string {
        $mtype = $confirmed ? self::MTYPE_CONFIRMED_DOWN : self::MTYPE_UNCONFIRMED_DOWN;
        $mhdr = chr(($mtype << 5) & 0xE0);
        $fctrl = ($adr ? 0x80 : 0x00) | ($ack ? 0x20 : 0x00);
        $fhdr = $devAddr . chr($fctrl) . pack('v', $fcnt & 0xFFFF);
